...
const OnRecruitPage = () => {
  ...
  //자산 현황 조회
  const { data: myInvestment } = useQuery('myInvestment', () => getMyInvestment());
  ...
  const mutation = useMutation(
    (data: { propertyId: string; quantity: number }) => fundraiseProperty(data.propertyId, { quantity: data.quantity }),
    {
      onSuccess: async (variables) => {
        try {
          navigate('/recruit-complete', {
            state: { quantity: variables.quantity, propertyId: variables.property_id },
          });
          ...
          await setTrustLine(xrplClient, myWallet, 'GRD', variables.quantity.toString(), uploaderWallet.classicAddress);
          await sendToken(
            xrplClient,
            uploaderWallet,
  ...
  return (
    <div className="onRecruitContainer">
      <Header
        leftContent={<img src={arrow} alt="back" onClick={() => navigate(-1)} />}
        centerContent={
          <strong
            style={{
              fontSize: '20px',
